...
class SAP_features:
    ...
    def spectral_entropy(self, x, fs, window_size, hop_length):
        # Calculate all features
        t, f, Sxx, gof, fm, am, sd, ent, amp = calculate_features(x, window_size, hop_length, fs)
...
def calculate_features(x, window_size, hop_length, fs, num_tapers=2):
    ...
    for i in range(len(wav_smp)):
        ...
        # Compute entropy
        sumLog = np.sum(np.log(sonogram[10:,i])) / (f[f_index].sum()-10)
        sumSon = np.sum(sonogram[10:,i]) / (f[f_index].sum()-10)
        
        # Same as -log(sumLog / sumSon)
        entropy[i] = (np.log(sumSon - sumLog) / np.log2(f[f_index].sum()))

        # Amplitude
        amplitude[i] = np.log(sonogram[10:,i]).sum()

    return t, f[f_index], sonogram, goodness_of_fit, frequency_modulation, amplitude_modulation, spectral_derivative, entropy, amplitude

def sonogram_and_spectral_derivative(x, fs, window_size, hop_length):
    sample_size = np.floor(len(x) / hop_length).astype(int)
    tapers = signal.windows.dpss(window_size, 1.5, 2)
    size = len(x)
    f_notShifted = fftfreq(window_size, 1/fs)
    f = fftshift(f_notShifted)
    ...
    wav_smp = np.arange(size-window_size, step=hop_length).astype(int)
    t = np.arange(np.floor(size / hop_length)) * (hop_length/fs)
    for i in range(len(wav_smp)+np.floor(window_size/hop_length).astype(int)-1):
        samps = np.arange(wav_smp[i], np.floor(wav_smp[i] + window_size).astype(int))
        window1 = x[samps] * tapers[0]
        window2 = x[samps] * tapers[1]
        ...
        powSpect1 = fftshift(fft(window1))
        powSpect2 = fftshift(fft(window2))
        r1 = (np.abs(powSpect1)**2 + np.abs(powSpect2)**2)
        sonogram[:,i] = r1[f_index]
    ...
    return sonogram, spectral_derivative